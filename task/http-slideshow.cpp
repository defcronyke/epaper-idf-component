...
static void sta_got_ip_event_handler(void *handler_arg, esp_event_base_t base, int32_t id, void *event_data)
{
  ...
  // UBaseType_t stack_res = uxTaskGetStackHighWaterMark(wifi_task_handle);
  // ESP_LOGW(TAG, "!!! [ wifi task ] sta_got_ip_event_handler() -> STACK SIZE !!!: %d", stack_res);

  xTaskCreate(&epaper_idf_ota_task, epaper_idf_ota_task_name, epaper_idf_ota_task_stack_depth * 8, NULL, epaper_idf_ota_task_priority, &ota_task_handle);
...
extern "C" void http_slideshow_task(void *pvParameter)
{
  ...
  { /**< Deep sleep. */
    ...
    xTaskCreate(&epaper_idf_wifi_task, epaper_idf_wifi_task_name, epaper_idf_wifi_task_stack_depth * 8, EPAPER_IDF_WIFI_TASK_ACTION_CAST_VOID_P(wifi_task_action), epaper_idf_wifi_task_priority, &wifi_task_handle);
